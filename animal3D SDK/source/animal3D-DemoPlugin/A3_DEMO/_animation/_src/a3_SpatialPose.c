...
//-----------------------------------------------------------------------------
		...
		//  -> concat (matrix mult) them in the correct order
		//		-> v' = t + R * S * v (4x4 matrix)
		
		// TEMPORARY SOLUTION (for testing, don't keep this here, do the above!!!!!)
		...
		//	-> func for that: a3trigValid_sind();
...
//-----------------------------------------------------------------------------
		...
		//(vectors)
		//a3real4 = vec4
		//a3real4Sum(a3real4, a3real4) = vec4 + vec4
		...
		//a3real4Sum(spatialPose_out->scale.v, spatialPose_lhs->scale.v, spatialPose_rhs->scale.v); //multiplicative, not additive for scale
...
//-----------------------------------------------------------------------------
		...
		a3real2QuotientComp(spatialPose_out->scale.v, spatialPose_lhs->scale.v, spatialPose_rhs->scale.v);
...
//-----------------------------------------------------------------------------
		...
		//a3real4??? needs log-lerp apparently (addition -> multiplication, multiplication -> exponentiation | subtraction -> division, division -> radical)
